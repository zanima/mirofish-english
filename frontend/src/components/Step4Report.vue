            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content ready, but section may not be fully complete) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll position to keep button in the same place
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/Analyze problem:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extract prediction scenario
    const reqMatch = text.match(/Predict scenario:\s*(.+?)(?:\n|$)/)
    ...
    const subQSection = text.match(/### Analysis of sub-questions\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    // Extract relationship chain - full extraction, no limit
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract statistics
    const nodesMatch = text.match(/Total node count:\s*(\d+)/)
    const edgesMatch = text.match(/Total edge count:\s*(\d+)/)
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interview count (e.g., "5 / 9 simulated Agent")
    ...
    const reasonMatch = text.match(/### Interviewee Selection Reasons\n([\s\S]*?)(?=\n---\n|\n### Interview Transcript)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: 1. **Alumni_345 (index=1)**: as a Wuda alumni...
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If the first question starts with "1.", special handling needed
      ...
      // Extract answers - by Twitter and Reddit
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (compatible with old format: only one platform tag case)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy as default display only when not placeholder text
          if (interview.redditAnswer && interview.redditAnswer !== '(This platform did not receive a reply)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(This platform did not receive a reply)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // No platform tags (extremely old format), treat entire as answer
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotation (compatible with multiple quotation formats)
      const quotesMatch = block.match(/\*\*Key quotation:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Prefer match > "text" format
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Attempt to extract edge information (if any)
    ...
    // Attempt to extract node information (if any)
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Latest Key Facts Linked in Temporal Memory'),
          ...
          }, expandedFacts.value ? 'Show Less ▲' : `Show All ${props.result.facts.length} ▼`)
        ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          ...
          }, expandedEntities.value ? 'Show Less ▲' : `Show All ${props.result.entities.length} ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          ...
          }, expandedRelations.value ? 'Show Less ▲' : `Show All ${props.result.relations.length} ▼`)
        ]),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No active memory'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? 'Show Less ▲' : `Show All ${props.result.activeFacts.length} ▼`)
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Attempt to extract time information [time - time]
                  (() => {
          ...
          }, expandedHistorical.value ? 'Show Less ▲' : `Show All ${props.result.historicalFacts.length} ▼`)
        ]),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No related entities'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check if it is platform placeholder text
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(This platform did not receive a reply)' || t === '(This platform did not receive a reply)' || t === '[No reply]' || t === '(No reply from this platform)'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Prefer to try "QuestionX:" format
      const cnPattern = /(?:^|[\r\n]+)Question(\d+)[:]\s*/g
      ...
      // If not matched, fallback to "number." format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // If no number found or only one found, return the whole
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      if (!answer) return ''
      if (isPlaceholderText(answer)) return '(No reply from this platform)'
    ...
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (non-placeholder text) and content differs
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle button (only shown when there is a real dual-platform answer)
...
const renderMarkdown = (content) => {
  ...
  // Remove the leading second-level heading (## xxx), because the chapter title is already displayed in the outer layer
  ...
  // Handle line breaks - empty lines become paragraph separators, single line breaks become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean <p><br> immediately following block-level elements (caused by extra empty lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> before paragraph start tag that follows block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering of non-continuous ordered lists: when a single <ol> is separated by paragraph content, keep numbering incrementing
...
const getLogLevelClass = (log) => {
  ...
  // INFO use default color, not marked as success
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - chapter generation complete
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // Automatically expand newly generated chapter
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Ensure clearing loading state
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task is complete, scroll to top; otherwise scroll to bottom following latest log
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // If the content is long and contains markdown format, try to remove the thought process and return
