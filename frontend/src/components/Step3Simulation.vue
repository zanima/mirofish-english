          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- Generic fallback: unknown type or unhandled content -->
...
const actionIds = ref(new Set()) // Set of action IDs for deduplication
...
// Track the last round of each platform, used to detect changes and output logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect round changes of each platform separately and output logs
      ...
      // Check if the simulation is completed (determined by runner_status or platform completion status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check: if the backend hasn't updated runner_status yet, but the platform has reported completion
      // Determine by checking twitter_completed and reddit_completed status
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Detected that all platform simulations have ended')
        }
...
const checkPlatformsCompleted = (data) => {
  ...
  // Check the completion status of each platform
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // If at least one platform has completed, check if all enabled platforms have completed
  // Determine if a platform is enabled via actions_count (if count > 0 or running was true)
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Use all_actions to get the complete list of actions
      ...
      // Do not auto-scroll, let users freely view the timeline
