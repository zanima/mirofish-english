...
// Directly get maxRounds from query parameter during initialization to ensure child components can immediately obtain the value
...
const handleGoBack = async () => {
  // Before returning to Step 2, first close the running simulation
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Failed to close env, attempting force stop...`)
        try {
      ...
      // Environment not running, check if the process needs to be stopped
...
const handleNextStep = () => {
  // The Step3Simulation component will directly handle report generation and route navigation
...
const loadGraph = async (graphId) => {
  // When simulating, auto-refresh without showing full-screen loading to avoid flicker
  // Show loading when manually refreshing or initially loading
  if (!isSimulating.value) {
...
onMounted(() => {
  ...
  // Record maxRounds configuration (value already obtained from query parameter during initialization)
  if (maxRounds.value) {
