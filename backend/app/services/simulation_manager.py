"""
OASIS Simulator Manager
Manage parallel simulation of Twitter and Reddit dual platforms
Use preset scripts + LLM to intelligently generate configuration parameters
"""
...
class SimulationState:
    ...
    def to_simple_dict(self) -> Dict[str, Any]:
        """Simplified status dictionary (API return use)"""
        return {
            "simulation_id": self.simulation_id,
            "project_id": self.project_id,
            "graph_id": self.graph_id,
...
class SimulationManager:
    ...
    def __init__(self):
        ...
        # Simulation status cache in memory
    ...
    def _save_simulation_state(self, state: SimulationState):
        """Save simulation status to file"""
    ...
    def prepare_simulation(
        ...
        3. Use LLM to intelligently generate simulation configuration parameters (time, activity level, speaking frequency, etc.)
        4. Save configuration file and Profile file
        5. Copy preset scripts to simulation directory
        
        Args:
            simulation_id: Simulation ID
            simulation_requirement: Simulation requirement description (used for LLM to generate configuration)
            document_text: Original document content (used for LLM to understand background)
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No entities matching the criteria were found, please check if the graph is correctly built"
                self._save_simulation_state(state)
                return state
            ...
            # Pass graph_id to enable Zep retrieval, obtaining richer context
            ...
            # Set the file path for real-time saving (prefer Reddit JSON format)
            ...
            # Save Profile file (Note: Twitter uses CSV format, Reddit uses JSON format)
            # Reddit has already been saved in real-time during generation; saving again here to ensure completeness
            if progress_callback:
                progress_callback(
            ...
            # ========== Stage 3: LLM intelligent generation of simulation configuration ==========
            if progress_callback:
                progress_callback(
                    "generating_config", 0, 
            ...
            def config_progress(current_step, total_steps, message):
                if progress_callback:
                    stage_progress = int(current_step / max(total_steps, 1) * 100)
                    progress_callback(
                        "generating_config",
                        stage_progress,
                        message,
            ...
            logger.info(f"Simulation preparation completed: {simulation_id}, entities={state.entities_count}, profiles={state.profiles_count}")
