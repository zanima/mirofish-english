...
Run simulation in background and record each Agent's actions, support real-time status monitoring
"""
...
# Process ID (for stopping)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Independent rounds and time per platform
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """Include detailed information of recent actions"""
        result = self.to_dict()
...
class SimulationRunner:
    ...
    2. Parse run logs, record each Agent's actions
    3. Provide real-time status query interface
    4. Support pause/stop/continue operations
    """
    
# Run status storage directory
...
# Run status in memory
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Maximum simulation rounds (optional, to truncate overly long simulations)
        enable_graph_memory_update: bool = False,  # Whether to update activity to Zep graph
        ...
        Args:
            ...
            enable_graph_memory_update: Whether to dynamically update Agent activity to Zep graph
        ...
        try:
            ...
            # If a maximum round number is specified, add to command line arguments
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])
            
            # Create main log file to avoid process blocking due to full stdout/stderr pipe buffer
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set subprocess environment variables to ensure UTF-8 encoding on Windows
            # This can fix the issue where third-party libraries (e.g., OASIS) read files without specified encoding
            env = os.environ.copy()
            ...
            # Inject the simulation-step model selection into the subprocess environment
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # create new process group, ensuring all related processes can be terminated when the server shuts down
            )
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # After process ends, read log one last time
            if os.path.exists(twitter_actions_log):
            ...
            logger.error(f"Monitoring thread exception: {simulation_id}, error={str(e)}")
            state.runner_status = RunnerStatus.FAILED
            state.error = str(e)
            cls._save_run_state(state)
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform ran, only check that platform
                                    # If two platforms ran, both need to complete
                                ...
                                # Update round information (from round_end event)
                                    ...
                                    # Update each platform's independent round and time
                                    if platform == "twitter":
                                    ...
                                    # Overall round takes the maximum of the two platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determine if a platform is enabled by checking the existence of the corresponding actions.jsonl file
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        Args:
            ...
            timeout: Timeout waiting for process to exit (seconds)
        """
        if IS_WINDOWS:
            ...
            try:
                ...
                # If it hasn't ended after timeout, force send SIGKILL
    ...
    def get_all_actions(
        ...
        # If the platform-specific file does not exist, try to read the old single file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # The old format file should have a platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def get_actions(
        ...
        Args:
            simulation_id: Simulation ID
            limit: Limit on number of items returned
            offset: Offset
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean simulation run logs (to force restart simulation)
        ...
        Note: configuration file (simulation_config.json) and profile files will not be deleted
        ...
        # Clean running state in memory
        if simulation_id in cls._run_states:
            del cls._run_states[simulation_id]
        
        logger.info(f"Clean simulation logs completed: {simulation_id}, deleted files: {cleaned_files}")
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is content to clean (avoid empty processes printing useless logs)
        has_processes = bool(cls._processes)
        has_updaters = bool(cls._graph_memory_enabled)
        
        if not has_processes and not has_updaters:
            return  # No content to clean, silent return
        
        logger.info("Cleaning all simulation processes...")
        
        # First stop all graph memory updaters (stop_all will log internally)
        try:
            ZepGraphMemoryManager.stop_all()
        except Exception as e:
            logger.error(f"Stop graph memory updater failed: {e}")
            ...
            try:
                if process.poll() is None:  # Process still running
                    ...
                    try:
                        ...
                        # Process may no longer exist, try to terminate directly
                        try:
                            process.terminate()
                    ...
                    try:
                        ...
                        logger.warning(f"Updating state.json failed: {simulation_id}, error={state_err}")
                        
            except Exception as e:
                logger.error(f"Cleaning process failed: {simulation_id}, error={e}")
    ...
    def register_cleanup(cls):
        ...
        Called when Flask app starts, ensuring all simulation processes are cleaned up when the server shuts down
        """
        ...
        # In Flask debug mode, register cleanup only in the reloader child process (the actual running app process)
        # WERKZEUG_RUN_MAIN=true indicates it's the reloader child process
        # If not in debug mode, this environment variable is absent, but registration is still needed
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # In debug mode, register only in the reloader child process; in non-debug mode, always register
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered to prevent child processes from attempting again
            return
        
        # Save the original signal handler
        original_sigint = signal.getsignal(signal.SIGINT)
        original_sigterm = signal.getsignal(signal.SIGTERM)
        # SIGHUP only exists on Unix systems (macOS/Linux), Windows does not
        ...
        def cleanup_handler(signum=None, frame=None):
            """Signal handler: first clean simulation processes, then call original handler"""
            # Only log when there are processes to clean
        ...
        try:
            ...
            # If not in main thread, can only use atexit
            logger.warning("Unable to register signal handler (not in main thread), only using atexit")
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True indicates environment is alive, False indicates environment is closed
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Status detail dictionary, includes status, twitter_available, reddit_available, timestamp
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: When simulating both platforms, interview both platforms simultaneously, return integrated result
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, will be overridden by each interview item's platform)
                - "twitter": Default interview only Twitter platform
                - "reddit": Default interview only Reddit platform
                - None: When simulating both platforms, each Agent interviews both platforms simultaneously
            timeout: Timeout duration (seconds)
    ...
    def interview_all_agents(
        ...
        if not agent_configs:
            raise ValueError(f"No Agent in simulation configuration: {simulation_id}")
    ...
    def close_simulation_env(
        ...
        """
        Close simulation environment (instead of stopping simulation process)
        
        Send close environment command to simulation, allowing it to gracefully exit wait command mode
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment shutdown command sent (waiting for response timeout, environment may be shutting down)",
            }
    ...
    def get_interview_history(
        ...
        """
        Get Interview history records (from database read)
        
        Args:
            simulation_id: Simulation ID
            platform: Platform type (reddit/twitter/None)
                - "reddit": Only get history from Reddit platform
                - "twitter": Only get history from Twitter platform
                - None: Get all history from both platforms
            agent_id: Specify Agent ID (optional, only get history for that Agent)
            limit: Number of results returned per platform
        ...
        # Determine which platform to query
        ...
        # If multiple platforms queried, limit total
